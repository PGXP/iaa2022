 ...
 * @author Serpro
 */
public class GrupoAllRunner implements Runnable {

    private static final Logger LOG = getLogger(GrupoAllRunner.class.getName());
...
//            LOG.log(Level.INFO, "Work {0} -> {1} -> {2}", new Object[]{estacao.getCodigo().toString(), lista.size(), between(start, finish).getNano()});
            ...
            Logger.getLogger(GrupoAllRunner.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
